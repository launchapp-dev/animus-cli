...
pub(crate) async fn handle_status(hub: Arc<dyn ServiceHub>, project_root: &str, json: bool) -> Result<()> {
    ...
    let dashboard = StatusDashboard {
        ...
        now: build_now_slice(tasks.as_deref(), now_error),
...
fn build_now_slice(tasks: Option<&[OrchestratorTask]>, error: Option<String>) -> NowSlice {
    if let Some(tasks) = tasks {
        let now = Utc::now();

        let next_task = find_next_ready_task(tasks);
...
fn find_next_ready_task(tasks: &[OrchestratorTask]) -> Option<NextTaskEntry> {
...
fn render_status_dashboard(dashboard: &StatusDashboard) -> String {
    ...
    for blocked in &dashboard.now.blocked_tasks {
        ...
        let _ = writeln!(&mut output, "    - {} ({}): {}", blocked.id, blocked.title, reason);
    }
