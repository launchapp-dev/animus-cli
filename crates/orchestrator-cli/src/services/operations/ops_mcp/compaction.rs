...
pub(super) fn compact_mcp_value(value: Value) -> Value {
    match value {
        Value::Array(items) => Value::Array(items.into_iter().map(compact_mcp_value).collect()),
        Value::Object(map) => Value::Object(
            map.into_iter()
                .map(|(key, value)| (key, compact_mcp_value(value)))
                .collect(),
        ),
        Value::String(text) => Value::String(compact_json_text(text)),
        other => other,
    }
}
