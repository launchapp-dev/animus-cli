...
impl AoMcpServer {
    ...
    #[tool(
        ...
        description = "Update daemon configuration. Purpose: Persist daemon automation settings and runtime-reconfigurable settings (pool_size, interval_secs, max_tasks_per_tick, auto_run_ready, stale_threshold_hours, phase_timeout_secs, notification_config_json, notification_config_file, clear_notification_config). Runtime settings are hot-reloaded by the running daemon without restart. Prerequisites: None. Example: {\"pool_size\": 4, \"interval_secs\": 10, \"auto_run_ready\": true}. Sequencing: Use animus.daemon.config to read current values first.",
