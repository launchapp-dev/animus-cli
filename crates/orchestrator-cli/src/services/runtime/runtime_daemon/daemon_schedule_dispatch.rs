...
mod tests {
    ...
    fn active_hours_gate_skips_due_schedules() {
        ...
        assert!(
            !dispatched,
            "schedules must not dispatch outside active hours"
        );
    }
    ...
    fn no_gating_when_active_hours_unset() {
        let time = chrono::NaiveTime::from_hms_opt(3, 0, 0).unwrap();
        assert!(
            ScheduleDispatch::allows_proactive_dispatch(None, time),
            "unset active_hours should not gate"
        );
    }
}
