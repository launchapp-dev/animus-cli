...
use crate::shared::collect_json_payload_lines;
...
impl PhaseFailureClassifier {
    ...
    pub(crate) fn provider_exhaustion_reason_from_text(text: &str) -> Option<String> {
    ...
    pub(crate) fn summarize_phase_diagnostics(lines: &VecDeque<String>) -> Option<String> {
        if lines.is_empty() {
            return None;
        }
