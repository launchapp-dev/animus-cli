...
pub(crate) struct PhaseExecutionMetadata {
    ...
    pub(super) phase_definition_hash: String,
    pub(super) agent_runtime_config_hash: String,
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub(super) agent_profile_hash: Option<String>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub(super) selected_tool: Option<String>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
pub(crate) struct PhaseExecutionRunResult {
    pub(super) outcome: PhaseExecutionOutcome,
    pub(super) metadata: PhaseExecutionMetadata,
    #[serde(default)]
    pub(super) signals: Vec<PhaseExecutionSignal>,
}
...
fn phase_ao_mcp_server_for(
    project_root: &str,
    phase_id: &str,
) -> Option<orchestrator_core::agent_runtime_config::AgentMcpServerConfig> {
    load_agent_runtime_config(project_root)
        .phase_agent_mcp_server(phase_id, "ao")
        .cloned()
}
...
fn inject_phase_profile_mcp_contract(
    ...
    if let Some(endpoint) = server
        ...
        .filter(|value| !value.is_empty())
    {
        mcp.insert("endpoint".to_string(), serde_json::json!(endpoint));
    } else {
        mcp.remove("endpoint");
    }

    let explicit_stdio = server.stdio.as_ref().and_then(|stdio| {
        let command = stdio.command.trim();
        if command.is_empty() {
            return None;
        }
        Some((command.to_string(), stdio.args.clone()))
    });

    let stdio = explicit_stdio.or_else(|| {
        if !matches!(
            server.source,
            Some(orchestrator_core::agent_runtime_config::AgentMcpServerSource::Builtin) | None
        ) {
            return None;
        }
    ...
    if let Some((command, args)) = stdio {
        mcp.insert(
            "stdio".to_string(),
            serde_json::json!({
                "command": command,
...
pub(super) fn build_phase_prompt(
    project_root: &str,
    workflow_id: &str,
    task_id: &str,
    task_title: &str,
...
pub(super) fn phase_requires_commit_message(phase_id: &str) -> bool {
...
pub(super) fn parse_research_reason_from_payload(payload: &Value) -> Option<String> {
    match payload {
...
pub(super) fn parse_research_reason_from_text(text: &str) -> Option<String> {
...
#[cfg(test)]
pub(super) fn parse_commit_message_from_payload(payload: &Value) -> Option<String> {
...
#[cfg(test)]
pub(super) fn parse_commit_message_from_text(text: &str) -> Option<String> {
    parse_commit_message_from_text_for_kind(text, "implementation_result")
}

pub(super) fn fallback_implementation_commit_message(task_id: &str, task_title: &str) -> String {
...
pub(super) async fn run_workflow_phase_with_agent(
    project_root: &str,
    execution_cwd: &str,
    workflow_id: &str,
    task_id: &str,
    ...
    for (target_index, (target_tool_id, target_model_id)) in execution_targets.iter().enumerate() {
        ...
        if let Some(mut runtime_contract) = build_runtime_contract(
            ...
            inject_phase_profile_mcp_contract(project_root, phase_id, &mut runtime_contract);
...
pub(super) async fn run_workflow_phase(
    project_root: &str,
    execution_cwd: &str,
    workflow_id: &str,
    task_id: &str,
...
mod tests {
    ...
    fn inject_phase_profile_mcp_contract_sets_builtin_ao_server_and_policy() {
        ...
        config
            ...
            .insert(
                ...
                orchestrator_core::agent_runtime_config::AgentMcpServerConfig {
                    ...
                    stdio: None,
                    tool_policy: Some(orchestrator_core::agent_runtime_config::AgentToolPolicy {
                        mode: Some(
                            orchestrator_core::agent_runtime_config::AgentToolPolicyMode::Allowlist,
                        ),
        ...
        let stdio_args = runtime_contract
            .pointer("/mcp/stdio/args")
            .and_then(Value::as_array)
            .expect("stdio args");
        let stdio_args = stdio_args
            .iter()
            .filter_map(Value::as_str)
            .collect::<Vec<_>>();
        assert!(stdio_args.contains(&"--project-root"));
        assert!(stdio_args.contains(&project_root.as_str()));
        assert!(stdio_args.contains(&"mcp"));
        assert!(stdio_args.contains(&"serve"));
        assert_eq!(
            runtime_contract
