...
pub async fn execute_workflow(mut params: WorkflowExecuteParams) -> Result<WorkflowExecuteResult> {
    ...
    // Record requirement success status when workflow completes
    if workflow.status == WorkflowStatus::Completed {
        project_requirement_success_status(hub.clone(), &workflow.subject, &workflow_ref).await?;
    }

    let total_duration = workflow_start.elapsed();
    let post_success = serde_json::json!({
        "status": "skipped",
        "reason": "post-success automation has been removed; git actions must be expressed as explicit workflow phases",
    });
