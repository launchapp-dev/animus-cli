...
use crate::executor::phase_executor::load_agent_runtime_config;
...
fn collect_phase_skills(project_root: &str, phase_id: &str) -> Vec<String> {
    ...
    if let Some(phase_skills) = rt_config
    ...
    if let Some(id) = agent_id.as_deref() {
        let wf_profile_skills = wf_config
            .config
            .agent_profiles
            .get(id)
            .map(|p| &p.skills)
        ...
        if let Some(profile_skills) = rt_config
            .agent_profile(id)
            .map(|p| &p.skills)
            .filter(|s| !s.is_empty())
        {
...
pub fn resolve_and_apply_phase_skills(
    ...
    let skills = collect_phase_skills(project_root, phase_id);
    if skills.is_empty() {
        return None;
    }
