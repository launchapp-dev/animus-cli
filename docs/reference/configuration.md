...
update `last-send.txt`. Events are flushed by `animus metrics flush` or
...
- manual reclaim sweep: `animus metrics cleanup` sweeps every repo scope under `~/.animus/`, deleting stale or oversized `flushing-*.jsonl` files and dropping oversized `pending.jsonl` buffers
...
| `animus metrics cleanup` | Sweep all scoped metrics dirs for orphaned/oversized flushing snapshots and oversized pending buffers. |
