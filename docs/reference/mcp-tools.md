...
| `animus.skill.create` | Author a project-scoped skill, written to `.animus/config/skill_definitions/<name>.yaml` and validated by round-tripping through the loader. Refuses to shadow an existing project skill unless `overwrite` is set. The result carries the same non-fatal `warnings` array as `animus.skill.get` when the written definition contains inert tool-id declarations | `project_root`, `name` (slug), `description`, `prompt`, `tags`, `tool_policy`, `model`, `mcp_servers`, `category`, `activation`, `capabilities`, `overwrite` |
