...
A skill name that does not resolve is a loud warning in the daemon dispatch log plus a `missing` record in the phase execution metadata — not a hard failure (the autonomous dispatch path never wedges on a stale skill reference, unlike the interactive `--skill` flag, which hard-errors). Misses of skills declared only by builtin agent-profile defaults (e.g. the `animus.core-skills` pack references) log at debug level instead.
